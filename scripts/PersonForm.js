...
var setNpcTraits = function(input, region){
    /*
    for(var i in input){
        if(input[i] == "$"){
            input[i] = "[Random]"
   ...
   "people":[  
    ...
    if(input[36] != null){
        person.firstName = input[36];    
    }
    if(input[37] != null){
        person.lastName = input[37];    
    }
    generateNpc(person);
}
...
var generateNpc = function(values){
   ...
   /* 
    ...
    for(var iz in TopDownGame.regNoArr){
        if(TopDownGame.regNoArr[iz] == person.region){
            person.regNo = iz
            break;
        }
    }
